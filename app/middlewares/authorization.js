...
async function verifyToken(req, res) {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ valid: false, message: "No hay token presente" });
    }
...
async function resetCardaLock(req, res, next) {
  ...
  if (loggeado.status) {
    ...
    connection.query(queryVer, function (error, results, fields) {
      ...
      if (results[0].id_priv === 0 || results[0].id_priv === 2) {
        ...
        res
          ...
          .send({
            status: "Error",
            message: "El usuario no tiene autorizacion para la peticion",
          });
    });

    for (let x in data) {
      const querySensorData = `SELECT * FROM instalaciones WHERE nombre = '${x}'`;
      console.log(x);
      ...
      try {
        ...
        const respuesta = await fetch(
          `http://${sensorIp}/resetBotes?password=imasd`,
        );
    ...
    res.send({
            status:"Ok",
            message: "Maquinas desbloqueadas"
        });
  }
}
  ...
  if (loggeado.status) {
    ...
    for (let v in data) {
      if (data[v] === null || data[v] === "") {
        ...
        res
          .status(400)
          .send({ status: "Error", message: "Error de entrada de Datos" });
      }
    }
    ...
    res
      .status(200)
      .send({ status: "ok", message: "Cambios efectuados correctamente" });
  }
}
...
async function formCalSent(req, res, next) {
  ...
  if (loggeado.status) {
    ...
    for (let x in data) {
      ...
      if (
        data["valor_calibracion_temperatura"] &&
        data["valor_calibracion_humedad"]
      )
        dataQuery.push(`temperatura = ${fcT}, humedad = ${fcH}`);
      else if (data["valor_calibracion_temperatura"])
        dataQuery.push(`temperatura = ${fcT}`);
      else if (data["valor_calibracion_humedad"])
        dataQuery.push(`humedad = ${fcH}`);
...
function formLimFil(req, res, next) {
  ...
  connection.query(query, function (error, results, fields) {
    ...
    if (Object.keys(results).length > 0) {
      ...
      return res
        .status(400)
        .send({ status: "Error", message: "Error, la instalacion no existe" });
  });
...
async function formCalClima(req, res, next) {
  ...
  connection.query(query, function (error, results, fields) {
    ...
    if (Object.keys(results).length > 0) {
      ...
      res
        .status(400)
        .send({ status: "Error", message: "La instalación no existe!!" });
  });
}
...
async function edicionMarchaCompresores(req, res, next) {
  ...
  if (loggeado.status) {
    ...
    connection.query(query, function (error, results, field) {
      if (error) {
        ...
        return res
          ...
          .send({
            status: "Error",
            message: "Error de verificacion de usuario",
          });
      }
      ...
      if (results[0].id_priv === 0 || results[0].id_priv === 4) {
        ...
        return res
          ...
          .send({
            status: "Error",
            message:
              "El usuario no tiene permiso para realizar la accion requerida",
          });
      ...
      connection.query(querySent, async function (error, results, field) {
        if (error) {
          ...
          return res
            ...
            .send({
              status: "Error",
              message: "Error en la carga de base de datos",
            });
        }
        ...
        try {
          ...
          if (nodeRed.status !== 200) {
            console.error(
              `Error desde Node-RED: ${nodeRed.status} ${nodeRed.statusText}`,
            );
          ...
          return res.status(200).send({
            status: "Ok",
            message:
              "Cambios efectuados correctamente y confirmados por Node-RED.",
            servicio: respuesta,
          });
        } catch (error) {
          if (error.name === "AbortError") {
            ...
            return res.status(500).send({
              status: "Error",
              message:
                "Error al intentar contactar con el servicio de Node-RED.",
            });
          }
        }
      });
    ...
    return res
      .status(400)
      .send({ status: "Error", message: "Ususario no Loggeado al Sistema" });
  //const dataIn = re
}
  ...
  if (logueado.status) {
    ...
    if (req.body.button === "Fábrica 1") dato = "Fabrica 1";
      ...
      return res
        .status(400)
        .send(
          { status: "Error", message: "Error de datos" },
          redirect("/limites"),
        );
